...
export default function ProfileModal({ visible, onClose, onNext, userID }) {
  ...
  const handleFinish = async (values) => {
    try {
      ...
      //   (cat) => cat.id === values.service_category
      // );
      // if (selectedCategory) {
      //   fd.append("service_category", selectedCategory.name);
      // }
    ...
    >
      ...
      <Form form={form} layout="vertical" onFinish={handleFinish}>
        ...
        <Form.Item
          ...
          rules={[{ required: true, message: "Upload a verification photo" }]}
        >
        ...
        <Form.Item
          ...
          valuePropName="fileList"
          getValueFromEvent={normFile}
          rules={[{ required: true, message: "Upload business proof" }]}
        >
          <Upload.Dragger {...uploadProps}>
            <p className="ant-upload-drag-icon">
              <InboxOutlined style={{ color: "#6961AB" }} />
        ...
        <Form.Item
          ...
          valuePropName="fileList"
          getValueFromEvent={normFile}
          rules={[{ required: true, message: "Upload Aadhaar copy" }]}
        >
          <Upload.Dragger {...uploadProps}>
            <p className="ant-upload-drag-icon">
              <InboxOutlined style={{ color: "#6961AB" }} />
        ...
        <Form.Item
          ...
          valuePropName="fileList"
          getValueFromEvent={normFile}
          rules={[{ required: true, message: "Upload PAN copy" }]}
        >
          <Upload.Dragger {...uploadProps}>
            <p className="ant-upload-drag-icon">
              <InboxOutlined style={{ color: "#6961AB" }} />
        ...
        <Form.Item name="website" label="Business Website (optional)">
          <Input placeholder="https://your-business.com" />
        </Form.Item>

        <Form.Item name="gstin" label="GSTIN No.">
