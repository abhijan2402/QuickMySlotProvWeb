...
export default function PrivacyPolicy() {
  ...
  if (isLoading) {
    return (
      <section className="py-28 flex flex-col items-center justify-center h-[700px] text-gray-600">
        <div className="relative">
          {/* Rotating Circle */}
          <div className="w-16 h-16 border-4 border-gray-200 border-t-purple-500 rounded-full animate-spin"></div>
          {/* Pulse Background */}
          <div className="absolute inset-0 w-16 h-16 bg-gradient-to-r from-purple-50 to-pink-50 rounded-full animate-pulse opacity-75"></div>
        </div>
        ...
        <p className="mt-1 text-sm text-gray-400">
          Please wait while we fetch the latest policy content
        </p>
      </section>
    );
  }
